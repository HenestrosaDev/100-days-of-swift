...
class GameScene: SKScene {
    ...
    func createFirework(xMovement: CGFloat, x: Int, y: Int) {
        ...
        // follow() makes the node move along the path specified as path.cgPath. asOffset makes the path start where the sprite starts. orientToPath makes the firework move towards the path
    ...
    func checkTouches(_ touches: Set<UITouch>) {
        ...
        // "case let as" checks if the element inside the array (nodesAtPoint) can be casted as SKSpriteNode. If so, then we go into the loop. node is the SKSpriteNode that the player just selected.
        for case let node as SKSpriteNode in nodesAtPoint {
            guard node.name == "firework" else { continue }
            
            // This loop ensures that the player selects only one firework at a time. If they select red then another red, both are selected but, if they select a green instead of a red, we deselect the first red because we need two reds.
    ...
    // Here, we handle those fireworks that the player doesn't destroy. If they past 900 points up vertically, then we remove them from the array and the parent. We nwws to move down backwards because if we remove the 3rd element, then the 4th element moves down to become 3.
