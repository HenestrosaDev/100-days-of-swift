...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        button1.layer.borderWidth = 1 //the measure is in points. for retina devices, it's the same as 2px. For retina HD devices, is 3px.
        button2.layer.borderWidth = 1
        button3.layer.borderWidth = 1
        
        //We can't put UIColor.lightGray property directly into our border color property because it belongs to a CA layer, which doesn't know where UI color is. That's why we convert it into a cgColor.
    ...
    func askQuestion(action: UIAlertAction? = nil) {
        ...
        //Looks like a enumm but it's actually but is actually a static property of a struct called UIControlState. In C, it's an enum but in C it gets mapped to a struck that just happens to be used like an enum.
    ...
    @IBAction func buttonTapped(_ sender: UIButton) {
        ...
        //VERY VERY IMPORTANT: We use askQuestion as the handler instead of askQuestion() because we use it as a reference. It will be executed when we tap the action. If we called the function instead, we wold be executing the function as soon as the compiler compiles (forgive the redundancy) it.
        if questionNumber + 1 <= 10 {
            alertController.addAction(UIAlertAction(title: "Continue", style: .default, handler: askQuestion))
        } else {
            alertController.addAction(UIAlertAction(title: "Bye bye", style: .default,
        ...
        //It's like the show() method. We have to specify which alertController we want to show and if we want it animated or not.
