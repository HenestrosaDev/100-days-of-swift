...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    /**
     We set the UUID of a iBeacon that we already know what UUID it will have, so if we approach some other beacon with another UUID towards the device, it won't be recognized.
    ...
    func startScanning() {
        ...
        beacons.append(CLBeaconRegion(proximityUUID: UUID(uuidString: "5A4BCFCE-174E-4BAC-A814-092E77F6B7E5")!, major: 123, minor: 456, identifier: "MyBeacon1"))
        ...
        beacons.append(CLBeaconRegion(proximityUUID: UUID(uuidString: "5A4BCFCE-174E-4BAC-A814-092E77F6B7E6")!, major: 456, minor: 789, identifier: "MyBeacon2"))
        beacons.append(CLBeaconRegion(proximityUUID: UUID(uuidString: "5A4BCFCE-174E-4BAC-A814-092E77F6B7E7")!, major: 654, minor: 321, identifier: "MyBeacon3"))
    ...
    func update(distance: CLProximity, beaconIdentifier: String) {
        UIView.animate(withDuration: 1) {
            switch distance {
            case .far:
                self.setDistance(transform: CGAffineTransform(scaleX: 0.25, y: 0.25), color: .blue, distance: "Far")
                
            case .near:
                self.setDistance(transform: CGAffineTransform(scaleX: 0.5, y: 0.5), color: .orange, distance: "Near")
                
            case .immediate:
                self.setDistance(transform: CGAffineTransform(scaleX: 1, y: 1), color: .red, distance: "Right here")
                
            default:
                self.setDistance(transform: CGAffineTransform(scaleX: 0.001, y: 0.001), color: .gray, distance: "Unknown")
            }
    ...
    func showAlertController() {
        let ac = UIAlertController(title: "A wild beacon appeared!", message: "For the first time", preferredStyle: .alert)
