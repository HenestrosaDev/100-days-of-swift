...
class ViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // Looks for a file named `start.txt` in the bundle of the project
        if let startWordsURL = Bundle.main.url(forResource: "start", withExtension: "txt") {
            /**
             We parse the elements of the start file to String. With contentsOf we say that we are
             going to extract those strings from startWorldsURL (the path of the file)
             */
    ...
    func submit(_ answer: String) {
        ...
        if isPossible(word: lowerAnswer)
            && !isRepeated(word: lowerAnswer)
            && isReal(word: lowerAnswer)
            && isShort(word: lowerAnswer)
            && !isExplicitlyInTheWord(word: lowerAnswer)
        {
            /**
             If the answer is valid, then we add it to the usedWords array and add it to the top of
             the table (indexPath 0 at 0)
             */
            ...
            if isExplicitlyInTheWord(word: lowerAnswer) {
                showErrorMessage(
                    title: "\(answer) is explicit in \(title)",
                    message: "The game would be too easy this way, don't you think?"
                )
            }
            ...
            if !isShort(word: lowerAnswer) {
                showErrorMessage(
                    ...
                    message: "It has to be larger than 2 characters"
                )
            }
    ...
    func isPossible(word: String) -> Bool {
        ...
        for letter in word {
            if let position = wordTmp.firstIndex(of: letter) {
                /**
                 If a letter of the input word is found in the wordTmp, then we remove the founded
                 letter from the wordTmp, so we can't use the same letter twice
                 */
    ...
    func isReal(word: String) -> Bool {
        ...
        /**
         We need to convert Swift strings to Objective-C strings. We need to use 16-bit Unicode
         transformation format because: Swift "é" and Objc "e´". UIKit was written in Objc so we
         need to use its codification of Strings.
         */
