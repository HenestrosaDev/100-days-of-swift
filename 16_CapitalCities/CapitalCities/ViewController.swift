...
//WARNING: It's requiered to hold CONTROL key on the Main.storyboard and drag the MapView to the ViewController and check the "delegate" option.
class ViewController: UIViewController, MKMapViewDelegate {
    ...
    //An annotation view works like a table view cell or a collection view cell. iOS reuses annotations to make best use of memory. This method will be responsible of assigning details to an annotation and of creating one if there isn't a dequeued one yet.
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        
        //This way, we avoid showing annotations that isn't ours. For example, if we enable tracking the user's location, then it will be showed as an annotation, which is not a Capital.
        ...
        //If it is the first annotation, then we set it up
        if annotationView == nil {
            annotationView = MKPinAnnotationView(annotation: annotation, reuseIdentifier: identifier)
            //If we set canShowCallout to true, then when we tap on an annotation, a popup will appear showing the information of the annotation. In this case, it will show the name of the capital
            annotationView?.canShowCallout = true
            //Challenge of the instructor
            annotationView?.pinTintColor = UIColor(red: 0.5, green: 1, blue: 0.5, alpha: 1)
            
            let btn = UIButton(type: .detailDisclosure)
            //Adds the button to the annotation. It will appear as a blue (i) because of .detailDisclosure
